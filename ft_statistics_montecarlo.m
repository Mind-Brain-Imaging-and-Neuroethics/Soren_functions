...
if ~isfield(cfg,'nanrand') || strcmpi(cfg.nanrand,'no')
resample = resampledesign(cfg, design);
Nrand = size(resample,1);
elseif strcmpi(cfg.nanrand,'yes') % only supports 'single-subject' stats
   newdesigns = nanresample(cfg,design,dat);
   Nrand = length(newdesigns);
end
...
if strcmp(cfg.correctm, 'cluster')
    ...
    elseif strcmp(cfg.clusterthreshold, 'parametric') && isempty(cfg.clustercritval)
        ...
        if isfield(cfg,'mediator'), tmpcfg.mediator = cfg.mediator; end %added by SWT 14/03/2019 for doing cluster correction of mediation analysis
        if isfield(cfg,'mxdesign'), tmpcfg.mxdesign = cfg.mxdesign; end %added by SWT 17/03/2019 for doing cluster correction on mixed effects regressions
        if isfield(cfg,'mxformula'), tmpcfg.mxformula = cfg.mxformula; end %added by SWT 17/03/2019 for doing cluster correction on mixed effects regressions
...
if strcmp(cfg.correctm, 'cluster')
    statrand = zeros(size(statobs,1), Nrand);
else
...
% modified by SWT to allow for parallel processing
if ~isfield(cfg,'parpool')
    for i=1:Nrand
        ...
        if strcmp(cfg.resampling, 'permutation')
            ...
            elseif strcmpi(cfg.nanrand,'yes')
                tmpdesign = newdesigns(i); % input the cell itself so the statfuns can recognize it and so it's distinct from a within-subjects design
            end
            tmpdat    = dat;  % the data itself is not shuffled

%             uniquevals = unique(tmpdesign);
%             for c = 1:length(uniquevals)
%                 for cc = 1:size(dat,1)
%                     if ~any(~isnan(dat(:,find(tmpdesign == uniquevals(c)))))
%                     
%                     end
%                end
%             end
...
function [newdesigns] = nanresample(cfg,design,dat)
% Added by SWT on 23/05/2019 to facilitate cluster testing with ECoG data 
% only supports permutation method with 'single-subject' design
% resamples NaNs and real values separately for each channel so that the
% sample size is kept constant for the permutations

for c = 1:size(dat,1)
    nanindices = find(isnan(dat(c,:)));
    realindices = find(~isnan(dat(c,:)));
    nreal = length(realindices);
    nnan = length(nanindices);
    for cc = 1:cfg.numrandomization
        newdesigns{cc}(c,realindices) = design(realindices(randperm(nreal)));
        newdesigns{cc}(c,nanindices) = design(nanindices(randperm(nnan)));
    end
end
