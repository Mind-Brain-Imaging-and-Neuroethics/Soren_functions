function [s, cfg] = ft_statfun_mixedeff(cfg, dat, design)

tmp = unique(design);
indices{1} = find(design == tmp(1));
indices{2} = find(design == tmp(2));

designMat = [dat(1,indices{1})' dat(1,indices{2})' cfg.mxdesign];
designTbl = array2table(designMat,'VariableNames',{'ind','dep','subs'});

categorical = (length(unique(dat(1,indices{2}))) == 2);

for c = 1:size(dat,1)
    designTbl.ind = dat(c,indices{1})';
    if categorical
       model = fitglme(designTbl,cfg.mxformula,'Distribution','Binomial','Link','logit');
    else
        model = fitlme(designTbl,cfg.mxformula);
    end
    %s.stat = -log(p)';
    p(c) = model.Coefficients.pValue(2);
end
s.stat = (1-p)';


%if cfg.tail == 0
    %s.critval = -log(cfg.alpha);
    s.critval = 0.95;
    s.df = size(dat,2)-2;
%end
end
