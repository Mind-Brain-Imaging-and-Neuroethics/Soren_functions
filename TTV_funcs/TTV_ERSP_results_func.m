...
save(fullfile(settings.outputdir,[settings.datasetname '_allcalc.mat']),'allmeas','-v7.3')
...
for q = 1:numbands
    alloutputs.ersp.pt.sig(q,:) = signrank_mat(allmeas{q}.naerspindex,zeros(size(allmeas{q}.naerspindex)),find(size(allmeas{q}.naerspindex) == min(size(allmeas{q}.naerspindex))));
