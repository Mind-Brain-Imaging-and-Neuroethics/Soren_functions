function [mdl] = Plot_figure_oscifrac_linreg(spec,fbands,opts)
...
for i = 1:length(fields)
    for c = 1:length(spec.(fields{i}))
        ...
        bb.(fields{i})(c,:) = IRASAPower_EEG_wrapper(spec.(fields{i})(c),'frac',opts.frange_ple,0,'mean');
    end
end
...
%% Transform into differences between groups/differences to reference category

newfields = cellcat(fields{1},fields(2:end),'_',0);

for i = 1:length(newfields)
    relmixd.(newfields{i}) = relmixd.(fields{i+1}) - relmixd.(fields{1});
    absmixd.(newfields{i}) = absmixd.(fields{i+1}) - absmixd.(fields{1});
    
    relosci.(newfields{i}) = relosci.(fields{i+1}) - relosci.(fields{1});
    absosci.(newfields{i}) = absosci.(fields{i+1}) - absosci.(fields{1});
    
    absfrac.(newfields{i}) = absfrac.(fields{i+1}) - absfrac.(fields{1});
    
    ple.(newfields{i}) = ple.(fields{i+1}) - ple.(fields{1});
    bb.(newfields{i}) = bb.(fields{i+1}) - bb.(fields{1});
end

%% Regression of oscillatory and fractal parameters on mixed power differences

mdl = cell(length(newfields),size(fbands,1));

for i = 1:length(newfields)
    for c = 1:size(fbands,1)
        tbl = array2table([nanmean(absmixd.(newfields{i})(:,:,c),2),nanmean(absosci.(newfields{i})(:,:,c),2),...
            nanmean(ple.(newfields{i}),2),nanmean(bb.(newfields{i}),2)],...
            'VariableNames',{['Mixed_' opts.fbandnames{c}] ['Osci_' opts.fbandnames{c}] 'PLE' 'Frac_BB'});
        tbl{:,:} = zscore(tbl{:,:},[],1);
        mdl{i,c} = fitlm(tbl,['Mixed_' opts.fbandnames{c} ' ~ Osci_' opts.fbandnames{c} ' + PLE + Frac_BB']);
    end
end 
...
for i = 1:size(fbands,1)
    p(2,i).select()
    for c = 1:length(newfields)
        scatter((1:length(mdl{c,i}.Coefficients.Estimate(2:end)))+c*0.15-0.15,mdl{c,i}.Coefficients.Estimate(2:end),36,pris(c,:));
        hold on
        er = errorbar((1:length(mdl{c,i}.Coefficients.Estimate(2:end)))+c*0.15-0.15,mdl{c,i}.Coefficients.Estimate(2:end),...
            1.96*mdl{c,i}.Coefficients.SE(2:end),'LineStyle','none','Color',pris(c,:),'LineWidth',2);
    end
    xax = get(gca,'XLim');
    line(xax,[0 0],'LineWidth',1.5,'Color',[0.5 0.5 0.5])
    set(gca,'XLim',xax+[-0.1 0.1],'XTickLabel',{['Osci ' opts.fbandnames{i}],'PLE','Frac BB'})
    ylabel('Coefficient')
    if length(newfields) > 1
        legend(replace(newfields,'_','-'))
    end
    FixAxes(gca,14)
...
%     scatter((1:size(stats_oscifrac.beta(:,c),1))+c*0.15-0.15,stats_oscifrac.beta(:,c),36,pris(c,:));
%     hold on
