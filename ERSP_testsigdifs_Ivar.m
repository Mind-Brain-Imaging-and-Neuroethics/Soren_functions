function [ersp,itc,erspboot,itcboot] = ERSP_testsigdifs_Ivar(conditions,electrode,varargin)
%currently you can only do pairwise comparisons

eeglab
%directory_name = uigetdir;
directory_name = '/Volumes/SOREN_SSD/Stuff_Ivar_SSD/eeg/raw_eeg/Self_conds';
cd(directory_name);
...
%if EasyParse(varargin,'TrialBased','true')
    ...
    [ersp,itc,~,~,~,erspboot,itcboot] = newtimef({ALLEEG(1).data(electrode,:,:) ALLEEG(2).data(electrode,:,:)},1500,[-1000 2000],500,0,'alpha',0.01);

%     for c = 1:size(ALLEEG(1).data,2)
%        for cc = 1:4
%           sigdifmat.(ALLEEG.chanlocs(cc).labels)(c) = ranksum(squeeze(ALLEEG(1).data(cc,c,:)),squeeze(ALLEEG(2).data(cc,c,:)));
%        end
%     end
%     
